...
(function() {
  ...
  const substringReplacements = [
    ...
    { search: 'Skills providing tailored instructions for happy path Dart and Flutter development workflows.', replace: '针对 Dart 和 Flutter 基础及常用开发工作流提供定制化的智能体指令集支持。' },
    { search: 'Configure the browser agent. Running this feature requires Google Chrome to be installed. The browser subagent can be invoked by typing /browser in the conversation input box.', replace: '配置浏览器智能体。运行此功能需要安装 Google Chrome 浏览器。可以通过在对话输入框中输入 /browser 来调用浏览器智能体。' },
    { search: 'Configure the browser agent. Running this feature requires', replace: '配置浏览器智能体。运行此功能需要安装' },
    { search: 'to be installed. The browser subagent can be invoked by typing', replace: '浏览器。可以通过在对话输入框中输入' },
