...
function createTray(actions) {
    ...
    // Add Click listener to instantly show and focus window on left click
    tray.on('click', () => {
        ...
        if (wins.length > 0) {
            ...
            const port = (0, utils_1.isMacOS)() ? require('./languageServer').getLsPort() : require('./languageServer').getLsPort();
            (0, utils_1.showOrCreateWindow)(port);
        }
    });

    // Add Double-Click listener to do the same
