...
function shouldRunInBackground() {
    try {
        const storagePath = path_1.default.join(electron_1.app.getPath('userData'), 'app_storage.json');
        ...
        const config = JSON.parse(content);
        const val = config?.runInBackground;
        return val === undefined
            ? process.platform !== 'win32'
            : val === 'true';
    }
    catch (e) {
        console.error('Error reading runInBackground setting synchronously:', e);
        return process.platform !== 'win32';
    }
}
/**
